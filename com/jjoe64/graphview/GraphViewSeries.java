...
public class GraphViewSeries {
	...
	static public class GraphViewSeriesStyle {
		...
		public GraphViewSeriesStyle(int color, int alpha, int thickness) {
			super();
			this.color = color;
			this.alpha = alpha;
			this.thickness = thickness;
	 ...
	 * 
	 * @param values
	 *            the values must be in the correct order! x-value has to be
	 *            ASC. First the lowest x value and at least the highest x
	 *            value.
	 */
	 ...
	 * @param style
	 *            custom style. can be null for default styles
	 * @param values
	 *            the values must be in the correct order! x-value has to be
	 *            ASC. First the lowest x value and at least the highest x
	 *            value.
	...
	public GraphViewSeries(String description, GraphViewSeriesStyle style,
			GraphViewDataInterface[] values, Bitmap bitmap, double suiteY) {
		super();
	...
	public GraphViewSeries(String description, GraphViewSeriesStyle style,
			GraphViewDataInterface[] values, KeyValuePair trendX, double trendY, Bitmap bitmap,
			double suiteY) {
		super();
		this.description = description;
		if (style == null) {
	 ...
	 * @param valueY
	 */
	public void addValue(KeyValuePair valueX, double valueY) {
		if (null != values && values.length > 0) {
			GraphViewDataInterface[] _values = new GraphViewData[values.length + 1];
			for (int i = 0; i < values.length; i++) {
				_values[i] = values[i];
			}
			_values[values.length] = new GraphViewData(valueX, valueY);
	...
	/**
	 * add one data to current data the values must be in the correct order!
	 * x-value has to be ASC. First the lowest x value and at least the highest
	 * x value.
	 * 
	 * @param value
	...
	public void appendData(GraphViewDataInterface value, boolean scrollToEnd) {
		// 6.1change-----change double getX changed to KeyValuePair getX.-----
		if (value.getX().getKey() < values[values.length - 1].getX().getKey()) {
	 ...
	 * 
	 * @param value
	 *            the new data to append. Important: the new value must be
	 *            higher then the last value (x).
	 * @param scrollToEnd
	 *            true => graphview will scroll to the end (maxX)
	 * @param maxDataCount
	...
	/**
	 * 6.1deprecated-----don't allow to throw exception.-----
	 */
	@Deprecated
	private void checkValueOrder() {
		// if (values.length > 0) {
		// // 6.1change-----change double getX changed to KeyValuePair
		// // getX.-----
		// double lx = values[0].getX().getKey();
		// for (int i = 1; i < values.length; i++) {
		// if (lx > values[i].getX().getKey()) {
		// throw new IllegalArgumentException(
